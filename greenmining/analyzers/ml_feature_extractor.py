...
class MLFeatureExtractor:
    ...
    def _default_keywords(self) -> List[str]:
        ...
        return [
            'cache', 'optimize', 'performance', 'efficient', 'reduce', 'compress',
            'lazy', 'async', 'parallel', 'batch', 'pool', 'scale', 'memory',
            'cpu', 'resource', 'green', 'sustainable', 'energy', 'power'
        ]
    ...
    def extract_temporal_features(self, commit: Dict, repo_commits: List[Dict]) -> Dict:
        ...
        recent_commits = [
            ...
            if self._parse_date(c.get('date')).timestamp() > week_ago
        ]
    ...
    def extract_features_batch(
        ...
        for i, commit in enumerate(commits):
            ...
            feature = self.extract_features(
                commit, repository, commits, analysis_results, label
            )
    ...
    def export_to_csv(self, features: List[MLFeatures], output_path: str) -> None:
        ...
        # Get all field names
        field_names = [
            'message_length', 'word_count', 'unique_word_ratio', 'avg_word_length',
            'has_green_keywords', 'keyword_count', 'keyword_density',
            'files_changed', 'lines_added', 'lines_deleted', 'total_changes', 'change_entropy',
            'hour_of_day', 'day_of_week', 'is_weekend', 'commit_velocity',
            'repo_stars', 'repo_age_days', 'primary_language',
            'author_green_rate', 'repo_green_rate',
            'is_green_aware'
        ]
        ...
        with open(output_path, 'w', newline='') as f:
            ...
            for feature in features:
                row = {
                    ...
                    'is_green_aware': int(feature.is_green_aware) if feature.is_green_aware is not None else '',
                }
                writer.writerow(row)
