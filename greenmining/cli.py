...
@click.option('--languages', default="Python,Java,Go,JavaScript,TypeScript", help='Comma-separated languages')
...
def extract(max_commits, skip_merges, days_back):
    ...
    try:
        ...
        colored_print(f"   Settings: max={max_commits}/repo, skip_merges={skip_merges}, days_back={days_back}\n", "cyan")
...
def analyze(batch_size):
    ...
    try:
        ...
        colored_print(f"   Method: Keyword-based heuristic analysis\n", "cyan")
...
def status():
    ...
    phases = {
        "1. Fetch Repositories": {
            ...
            "size": f"{config.REPOS_FILE.stat().st_size / 1024:.1f} KB" if config.REPOS_FILE.exists() else "N/A"
        },
        "2. Extract Commits": {
            "file": str(config.COMMITS_FILE),
            "completed": config.COMMITS_FILE.exists(),
            "size": f"{config.COMMITS_FILE.stat().st_size / 1024:.1f} KB" if config.COMMITS_FILE.exists() else "N/A"
        },
        "3. Analyze Commits": {
            "file": str(config.ANALYSIS_FILE),
            "completed": config.ANALYSIS_FILE.exists(),
            "size": f"{config.ANALYSIS_FILE.stat().st_size / 1024:.1f} KB" if config.ANALYSIS_FILE.exists() else "N/A"
        },
        "4. Aggregate Results": {
            "file": str(config.AGGREGATED_FILE),
            "completed": config.AGGREGATED_FILE.exists(),
            "size": f"{config.AGGREGATED_FILE.stat().st_size / 1024:.1f} KB" if config.AGGREGATED_FILE.exists() else "N/A"
        },
        "5. Generate Report": {
            "file": str(config.REPORT_FILE),
            "completed": config.REPORT_FILE.exists(),
            "size": f"{config.REPORT_FILE.stat().st_size / 1024:.1f} KB" if config.REPORT_FILE.exists() else "N/A"
        }
    }
...
def pipeline(max_repos, skip_fetch):
    ...
    try:
        ...
        save_json_file(results, config.ANALYSIS_FILE)
        colored_print(f"   Analyzed {len(results)} commits, saved to: {config.ANALYSIS_FILE}", "green")
