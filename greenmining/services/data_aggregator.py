...
class DataAggregator:
    ...
    def aggregate(self, analysis_results: List[Dict[str, Any]], repositories: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _generate_summary(self, results: List[Dict[str, Any]], repos: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        repos_with_green = len(set(r['repo_name'] for r in results if r.get('green_aware', False)))
        
        return {
            "total_commits": total_commits,
            "green_aware_count": green_aware_count,
            "green_aware_percentage": round(green_aware_count / total_commits * 100, 2) if total_commits > 0 else 0,
    ...
    def _analyze_known_patterns(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for pattern_name, data in sorted(pattern_data.items(), key=lambda x: x[1]['count'], reverse=True):
            patterns_list.append({
                ...
                "count": data['count'],
                "percentage": round(data['count'] / total_patterns * 100, 1) if total_patterns > 0 else 0,
    ...
    def _generate_repo_stats(self, results: List[Dict[str, Any]], repos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for repo_name, commits in repo_commits.items():
            ...
            patterns = [c.get('known_pattern') for c in commits if c.get('known_pattern') != 'NONE DETECTED']
            ...
            repo_stats.append({
                ...
                "percentage": round(len(green_commits) / len(commits) * 100, 1) if commits else 0,
    ...
    def _generate_language_stats(self, results: List[Dict[str, Any]], repos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def save_results(self, aggregated_data: Dict[str, Any], json_file: Path, csv_file: Path, analysis_results: List[Dict[str, Any]]):
        ...
        for result in analysis_results:
            csv_data.append({
                ...
                'green_evidence': result.get('green_evidence', '')[:200] if result.get('green_evidence') else '',
    ...
    def print_summary(self, aggregated_data: Dict[str, Any]):
        ...
        summary_table = [
            ...
            ["Green-Aware Commits", f"{format_number(summary['green_aware_count'])} ({format_percentage(summary['green_aware_percentage'])})"],
        ...
        if aggregated_data['known_patterns']:
            ...
            print(tabulate(pattern_table, headers=['Pattern', 'Count', '%', 'Confidence'], tablefmt='simple'))
        ...
        if aggregated_data['per_repo_stats']:
            ...
            print(tabulate(repo_table, headers=['Repository', 'Total', 'Green', '%'], tablefmt='simple'))
        ...
        if aggregated_data['per_language_stats']:
            ...
            print(tabulate(lang_table, headers=['Language', 'Total', 'Green', '%'], tablefmt='simple'))


@click.command()
@click.option('--analysis-file', default=None, help='Input analysis file (default: data/analysis_results.json)')
@click.option('--repos-file', default=None, help='Input repositories file (default: data/repositories.json)')
@click.option('--output-json', default=None, help='Output JSON file (default: data/aggregated_statistics.json)')
@click.option('--output-csv', default=None, help='Output CSV file (default: data/green_analysis_results.csv)')
@click.option('--config-file', default='.env', help='Path to .env configuration file')
def aggregate(analysis_file: Optional[str], repos_file: Optional[str], output_json: Optional[str], output_csv: Optional[str], config_file: str):
    ...
    try:
        ...
        colored_print(f"Loaded {len(analysis_results)} analysis results and {len(repositories)} repositories", "green")
