...
class GitHubFetcher:
    ...
    def __init__(self, token: str, max_repos: int = 100, min_stars: int = 100, languages: Optional[List[str]] = None):
        ...
        self.languages = languages or ["Java", "Python", "Go", "JavaScript", "TypeScript", "C#", "Rust"]
    ...
    def search_repositories(self) -> List[Dict[str, Any]]:
        ...
        colored_print(f"Filters: min_stars={self.min_stars}, languages={', '.join(self.languages)}", "cyan")
        ...
        try:
            ...
            with tqdm(total=min(self.max_repos, total_found), desc="Fetching repositories", unit="repo") as pbar:
...
@click.option('--languages', default='java,python,go,javascript,typescript,csharp,rust', 
...
def fetch(max_repos: int, min_stars: int, languages: str, output: Optional[str], config_file: str):
    ...
    try:
        ...
        table_data = [
            [repo['full_name'], repo['language'], f"{repo['stars']:,}", repo['description'][:50] + "..."]
            for repo in top_repos
        ]
        print(tabulate(table_data, headers=['Repository', 'Language', 'Stars', 'Description'], tablefmt='simple'))
