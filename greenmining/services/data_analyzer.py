...
from greenmining.gsf_patterns import GSF_PATTERNS, GREEN_KEYWORDS, get_pattern_by_keywords, is_green_aware
...
class DataAnalyzer:
    ...
    def analyze_commits(self, commits: List[Dict[str, Any]], resume_from: int = 0) -> List[Dict[str, Any]]:
        ...
        with tqdm(total=len(commits), initial=resume_from, desc="Analyzing commits", unit="commit") as pbar:
            for idx, commit in enumerate(commits[resume_from:], start=resume_from):
                try:
                    analysis = self._analyze_commit(commit)
                    results.append(analysis)
                    pbar.update(1)
                except Exception as e:
                    colored_print(f"\nError analyzing commit {commit.get('commit_id', 'unknown')}: {e}", "yellow")
                    pbar.update(1)
    ...
    def save_results(self, results: List[Dict[str, Any]], output_file: Path):
        ...
        pattern_counts = Counter(r['known_pattern'] for r in results if r['known_pattern'] != 'NONE DETECTED')
        ...
        data = {
            "metadata": {
                ...
                "green_aware_percentage": round(green_aware_count / len(results) * 100, 2) if results else 0,
        ...
        colored_print(f"  Green-aware commits: {green_aware_count} ({data['metadata']['green_aware_percentage']}%)", "white")
        if pattern_counts:
...
@click.option('--commits-file', default=None, help='Input commits file (default: data/commits.json)')
@click.option('--output', default=None, help='Output file path (default: data/analysis_results.json)')
@click.option('--config-file', default='.env', help='Path to .env configuration file')
def analyze(batch_size: int, resume: bool, commits_file: Optional[str], output: Optional[str], config_file: str):
    """Analyze commits for green software practices."""
    ...
    try:
        ...
        if resume:
            ...
            if checkpoint_data:
                ...
                colored_print(f"Resuming from checkpoint: {resume_from} commits processed", "yellow")
        ...
        # Save checkpoint
        create_checkpoint(config.CHECKPOINT_FILE, {
